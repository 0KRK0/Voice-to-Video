...
class ProjectRepository(Protocol):
    ...
    async def get_project(
        ...
        """One project. Passing ``organisation_id`` scopes the read to a tenant.

        Scoping in the query rather than after it is deliberate: a caller who
        forgets an `if` leaks another customer's data, and a query that cannot
        match does not.
        """
        ...
