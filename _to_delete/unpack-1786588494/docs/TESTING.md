...
Current state: **254 tests**, no network, no credentials, no provider access. The
contract and intelligence tests run in under four seconds; the rendering tests
add about thirty because they encode real video, and they skip cleanly when
ffmpeg is absent.
...
| --- | --- |
| `test_base.py` | Identifiers, time spans, storage refs, budgets |
| `test_asset_licensing.py` | Rights fail closed; scraping is impossible |
| `test_pipeline_contracts.py` | Transcript, understanding and scene invariants |
| `test_visual_plan.py` | Director contract, fallback ladders, no executable specs |
| `test_timeline.py` | Fit policies, coverage gaps, placeholders |
| `test_generation_and_errors.py` | Cache keys, retryability, honest results |
| `test_project_and_ports.py` | Project state, retention, port shapes |
| `test_schema_export.py` | Contract drift |
| `test_example_transistor.py` | The worked example and the product rules |
| `test_architecture_boundaries.py` | Vendor code stays in adapters |
| `test_capture_and_speech.py` | Magic-byte sniffing, probing, silence, alignment |
| `test_intelligence.py` | NLP, understanding, grouping, visual decisions |
| `test_generation_router.py` | Selection, retry, circuit breaking, budgets, privacy |
| `test_render_and_delivery.py` | Every primitive draws; degradation; real MP4 |
| `test_api_and_platform.py` | Persistence, jobs, wiring, `/v1/visualize`, golden path |
