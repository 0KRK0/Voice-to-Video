...
class Persistence(unittest.TestCase):
    ...
    def test_a_project_round_trips(self) -> None:
        project = Project(title="A talk about transistors")
    ...
    def test_expired_temporary_projects_are_identified(self) -> None:
        stale = Project(expires_at=utc_now() - timedelta(hours=1))
        fresh = Project(expires_at=utc_now() + timedelta(hours=1))
        saved = Project(persistence=PersistenceMode.SAVED)
...
class Wiring(unittest.TestCase):
    ...
    def test_settings_never_log_a_credential(self) -> None:
        settings = Settings(text_generation_api_key="super-secret-value")
