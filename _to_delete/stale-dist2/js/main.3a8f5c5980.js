 ...
 * 30ms.
 */
import { el, must } from "/js/core/dom.b9f550b45c.js";
import { Router } from "/js/core/router.7b322edb5d.js";
import { VtvClient } from "/js/api/client.0956cb21f5.js";
import { session, signIn } from "/js/core/session.89377f038d.js";
import { startScreen } from "/js/screens/start.f671958300.js";
import { projectsScreen } from "/js/screens/projects.b0c4587a3e.js";
import { studioScreen } from "/js/screens/studio.a58d9e1fec.js";
import { rendersScreen, settingsScreen } from "/js/screens/renders.0337b4c53e.js";
import { emptyState } from "/js/widgets/states.acb58b27e1.js";
...
async function boot() {
    ...
    router
        ...
        // The Studio is the one route that fills the viewport exactly rather than
        // growing with its content. See `body.is-app .shell`.
        .add("studio", "/studio/:projectId", (host, match) => appRoute(studioScreen(host, match.params.projectId ?? "", match.query, {
        client,
        router,
...
/**
 * Mark the document while an application route is mounted, and unmark it when
 * the route is torn down.
 *
 * The class is what switches the shell from "a page that grows with its
 * content" to "a pane exactly the height of the viewport". It has to come off
 * again on navigation, or the project list inherits a shell it cannot scroll.
 */
function appRoute(disposer) {
    document.body.classList.add("is-app");
    disposer.add(() => document.body.classList.remove("is-app"));
    return disposer;
}
function chrome() {
