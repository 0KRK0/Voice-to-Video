...
import type { MediaOrigin, VisualUnitStatus } from "../api/types.js";

export interface StateStyle {
  /** A glyph, so the state survives greyscale and colour blindness. */
  glyph: string;
  label: string;
  /** The CSS class carrying the colour and the fill. */
  tone: string;
  /** What the tooltip says. Written for a person. */
  hint: string;
}
 ...
 */
export const UNIT_STATES: Record<VisualUnitStatus, StateStyle> = {
  planned: {
    glyph: "○",
    label: "Planned",
    tone: "is-planned",
...
export const ORIGIN_STATES: Record<MediaOrigin, StateStyle> = {
  user_upload: {
    glyph: "",
    label: "Your media",
    tone: "origin-yours",
...
export function unitBadge(
  ...
  return el(
    ...
    state.glyph && el("span", { class: "state__glyph", "aria-hidden": "true" }, state.glyph),
    !options.compact && state.label,
  );
}

/** The badge every surface uses for where a picture came from. */
export function originBadge(origin: MediaOrigin): HTMLElement {
...
export function lockBadge(): HTMLElement {
  return el(
    ...
    el("span", { class: "state__glyph", "aria-hidden": "true" }, "⌂"),
    "Locked",
  );
}
...
export function emptyState(options: {
  ...
  return el(
    "div",
    { class: "empty" },
    el("h4", { class: "empty__title" }, options.title),
    el("p", { class: "empty__body" }, options.body),
    options.action &&
      el(
        "button",
        { class: "btn btn--primary", type: "button", onclick: options.action.onClick },
...
export function inlineError(
  ...
  return el(
    ...
    el("span", { class: "notice__text" }, message),
    action &&
      el(
        "button",
        { class: "btn btn--sm", type: "button", onclick: action.onClick },
        action.label,
      ),
  );
}
