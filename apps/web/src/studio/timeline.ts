...
import { el, on, toggle } from "../core/dom.js";
import { clamp, quantise, timecode } from "../core/format.js";
import { Disposer, framed } from "../core/store.js";
import type {
  EditTimeline,
  TimelineClip,
  TimelineOperation,
...
import { resolveSelection, type StudioState } from "./state.js";
...
export interface TimelineDeps {
  state: StudioState;
  /** Apply operations. Resolves to `true` when the server accepted them. */
  apply(
    operations: TimelineOperation[],
    options?: { describe?: string; anchor?: HTMLElement },
  ): Promise<boolean>;
  /** Move the playhead and pull fresh preview metadata. */
  seek(seconds: number): void;
  /** Drop a media asset onto a lane at a time. */
  dropMedia(assetId: string, kind: TrackKind, at: number): Promise<void>;
  /**
   * Show a refusal against the element it applies to.
   *
   * Passed in rather than called directly so that every refusal in the Studio —
   * the server's and this module's — goes to the same place and looks the same.
   */
  refuse(anchor: HTMLElement, sentence: string): void;
  /** Create a lane of this kind. */
  addLane(kind: TrackKind): Promise<void>;
}

export interface TimelineView {
  node: HTMLElement;
  /** Rebuild from the current timeline. Cheap enough to call on any change. */
  refresh(): void;
  /** Redraw only the playhead. Called on every animation frame of playback. */
  paintPlayhead(): void;
  dispose(): void;
}
...
export function createTimeline(deps: TimelineDeps): TimelineView {
  ...
  const scroller = el(
    "div",
    { class: "tl__scroll scroll", id: "tl-scroll" },
    el("div", { class: "tl__surface" }, ruler, lanes, scopeBand, snapGuide, playhead),
  );
  ...
  const addLane = el(
    "div",
    { class: "tl__addlane" },
    el("span", { class: "label muted" }, "Add lane"),
  ...
  const overview = el(
    "div",
    { class: "tl__ov", "aria-label": "Project overview" },
    overviewTrack,
    overviewWindow,
  );
  ...
  const node = el(
    "section",
    { class: "tl", "aria-label": "Timeline" },
    el(
      "header",
      { class: "tl__bar" },
      el("span", { class: "tl__clock tabular" }, "0:00.000"),
      el("div", { class: "spacer" }),
      addLane,
      viewportNote,
      fit,
    ...
    overview,
    el("div", { class: "tl__body" }, headers, scroller),
  );

  const clockNode = node.querySelector<HTMLElement>(".tl__clock");
  ...
  function paintRuler(): void {
    ...
    for (let t = Math.max(0, from); t <= to; t += minor) {
      ...
      if (isMajor) {
        ruler.appendChild(
          el(
            "span",
            { class: "tl__ticklabel tabular", style: { left: `${x + 3}px` } },
            timecode(t, millis),
          ),
        );
      }
    ...
    if (target) {
      ruler.appendChild(
        el(
          "span",
          { class: "tl__target", style: { left: `${map.x(target)}px` } },
          el("span", { class: "tl__target-label label" }, "Target"),
        ),
      );
    }
  ...
  function paintLanes(): void {
    ...
    const ordered = [...timeline.tracks].sort(
  ...
  function trackHeader(track: TimelineTrack): HTMLElement {
    return el(
      "div",
      {
      ...
      track.derived
        ? el(
            "span",
            {
              ...
              title:
                "Generated from your script. Edit the script instead — a change " +
        ...
        : null,
      el("div", { class: "spacer" }),
      track.kind === "music" || track.kind === "sfx" || track.kind === "narration"
        ? el(
            "button",
            {
  ...
  function laneFor(track: TimelineTrack, width: number): HTMLElement {
    const lane = el("div", {
      class: [
        "tl__lane",
        `is-${track.kind}`,
    ...
    if (visible.length > DENSITY_THRESHOLD) {
      for (const band of densityBands(visible, map)) lane.appendChild(band);
      return lane;
    }
    ...
    for (const clip of visible) lane.appendChild(clipNode(clip, track, map));
    return lane;
  }

  /** The origin of a unit's selected version, or `null` when it has none. */
  function originOf(unit: VisualUnit | undefined): string | null {
    if (!unit) return null;
      ...
      width >= 46 && originOf(unit)
        ? el(
            "span",
            {
              ...
              title: ORIGIN_WORDS[originOf(unit) as string] ?? "",
            },
            ORIGIN_MARKS[originOf(unit) as string] ?? "",
          )
        : null,
   ...
   */
  function densityBands(clips: TimelineClip[], map: Mapping): HTMLElement[] {
    ...
    while (index < clips.length) {
      ...
      while (index < clips.length) {
        ...
        if (!clip) break;
        if (map.x(clip.end) - left > bandPx && count > 0) break;
        if (clip.locked) locked += 1;
        const unit = clip.visual_unit_id
          ? state.unitById.get().get(clip.visual_unit_id)
          : undefined;
        ...
        if (unit?.status === "degraded") degraded += 1;
        if (previousEnd !== null && clip.start - previousEnd > 0.05) gaps += 1;
  ...
  const paintPlayhead = framed(() => {
    ...
    if (clockNode) clockNode.textContent = timecode(at);
  ...
  /**
   * Snap targets, in priority order, each with a name.
   *
   * The name is the point. "Snapped to 0:31.0" tells a user a number they can
   * already read off the readout; "snapped to line 04 start" tells them *why*
   * the clip stopped there, which is the only reason snapping is worth having.
   */
  ...
  let snapNames = new Map<number, string>();

  function snapTargets(exclude: string): SnapTarget[] {
    const timeline = state.timeline.get();
    ...
    for (const link of timeline?.links ?? []) {
      ...
      const label = number
        ...
        : "a script line";
      targets.push(
        { at: link.start, name: `${label} start` },
        { at: link.end, name: `${label} end` },
      );
    }

    for (const track of timeline?.tracks ?? []) {
      for (const clip of track.clips) {
        if (clip.clip_id === exclude) continue;
        const label = clip.label || "a clip";
        targets.push(
          { at: clip.start, name: `${label} start` },
          { at: clip.end, name: `${label} end` },
        );
      }
    }
    ...
    for (const target of targets) {
      ...
      if (!snapNames.has(key)) snapNames.set(key, target.name);
    }
    return targets;
  }

  function snapName(at: number): string {
    return snapNames.get(round3(at)) ?? timecode(at, false);
  }

  function snap(value: number, targets: SnapTarget[], suppressed: boolean): {
    at: number;
    hit: number | null;
  } {
    if (suppressed) return { at: value, hit: null };
  ...
  disposer.add(
    ...
    on(ruler, "pointerdown", (event) => {
      ...
      const scrub = (moveEvent: PointerEvent) => {
    ...
    on(lanes, "pointerdown", (event) => onLanePointerDown(event)),
    on(lanes, "dblclick", (event) => {
      const clip = clipAt(event.target);
      if (clip) deps.seek(Number(clip.dataset.start ?? 0));
    }),
    on(lanes, "keydown", (event) => onClipKey(event)),

    // Drag and drop from the media library.
    on(lanes, "dragover", (event) => {
      if (!event.dataTransfer?.types.includes("application/x-vtv-media")) return;
      event.preventDefault();
      event.dataTransfer.dropEffect = "copy";
      const lane = (event.target as Element).closest<HTMLElement>(".tl__lane");
      lanes.querySelectorAll(".is-droptarget").forEach((node) =>
    ...
    on(lanes, "drop", (event) => {
      ...
      void deps.dropMedia(assetId, (lane.dataset.kind ?? "visual") as TrackKind, at);
    }),
  );
   ...
   */
  function laneLevel(track: TimelineTrack): string {
  ...
  function paintOverview(): void {
    ...
    overviewWindow.setAttribute(
      "aria-valuetext",
      `${timecode(from, false)} to ${timecode(to, false)} of ${timecode(total, false)}`,
    );
  }

  /** Centre the viewport on a fraction of the project. */
  function scrollToFraction(fraction: number): void {
    const total = duration();
    ...
    scroller.scrollLeft = Math.max(
      0,
      map.x(at) - scroller.clientWidth / 2,
    );
    refresh();
  }
  ...
  disposer.add(
    on(overview, "pointerdown", (event) => {
      ...
      const grabbed = (event.target as Element).closest(".tl__ov-window") !== null;
      ...
      const drag = (moveEvent: PointerEvent): void => {
  ...
  function clipAt(target: EventTarget | null): HTMLElement | null {
    return (target as Element | null)?.closest?.<HTMLElement>(".tl__clip") ?? null;
  }
  ...
  function onLanePointerDown(event: PointerEvent): void {
    ...
    const lane = node.closest<HTMLElement>(".tl__lane");
    ...
    if (isDerived) {
      ...
      deps.refuse(
        node,
        `The ${kind} track comes from your script. Edit the script instead — ` +
    ...
    const move = on(window, "pointermove", (moveEvent) => {
      ...
      const describeSnap = (hit: number | null): string =>
        suppressed
          ? " · free"
          : hit === null
            ? ""
    ...
    const finish = async (): Promise<void> => {
      ...
      const operation: TimelineOperation = edge
  ...
  function showSnap(at: number | null): void {
    if (at === null) {
      snapGuide.hidden = true;
      return;
    }
   ...
   */
  function onClipKey(event: KeyboardEvent): void {
    const node = clipAt(event.target);
    const clipId = node?.dataset.clip;
    if (!node || !clipId) return;
    ...
    if (event.altKey && (event.key === "ArrowLeft" || event.key === "ArrowRight")) {
      ...
      void deps.apply(
        [{ kind: "move", clip_id: clipId, start: quantise(Math.max(0, start + delta)) }],
        { describe: "Nudge", anchor: node },
      );
      return;
    }
  ...
  async function toggleMute(track: TimelineTrack): Promise<void> {
    ...
    await deps.apply(
      [{ kind: "set_track", track_id: track.track_id, muted: !track.muted }],
      { describe: track.muted ? "Unmute lane" : "Mute lane" },
    );
  }
  ...
  function refreshSelection(): void {
    ...
    lanes.querySelectorAll<HTMLElement>(".tl__clip").forEach((node) => {
  ...
  disposer.add(
    state.timeline.subscribe(() => refresh()),
    state.units.subscribe(() => refresh()),
    state.selection.subscribe(() => refreshSelection()),
    state.playhead.subscribe(() => paintPlayhead()),
...
function button(
  ...
  return el(
    "button",
    { class: "btn btn--sm", type: "button", title, "aria-label": title, onclick: onClick },
    label,
  );
}

function labelFor(track: TimelineTrack): string {
 ...
 */
function tickSizes(pps: number): {
  major: number;
  minor: number;
  millis: boolean;
} {
  if (pps < 2) return { major: 60, minor: 10, millis: false };
  if (pps < 10) return { major: 30, minor: 5, millis: false };
  if (pps < 40) return { major: 10, minor: 1, millis: false };
  if (pps < 150) return { major: 1, minor: 0.1, millis: false };
