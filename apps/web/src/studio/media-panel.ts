...
import { bytes, clock, humanise } from "../core/format.js";
import { Disposer } from "../core/store.js";
import { emptyState, inlineError, originBadge, skeleton } from "../widgets/states.js";
import { confirm, openDialog, refuseAt, toast } from "../widgets/dialog.js";
import type { MediaAsset, MediaOrigin, TrackKind } from "../api/types.js";
...
export interface MediaPanelDeps {
  state: StudioState;
  client: VtvClient;
  /** Use an asset as the currently selected visual. */
  useAsVisual(assetId: string, lock: boolean): Promise<void>;
  /** Open the asset's own detail view. */
...
export interface MediaPanelView {
  node: HTMLElement;
  /** Open the library filtered to the user's own files, for "Replace from Media". */
  focusUploads(): void;
  dispose(): void;
}
 ...
 */
const TABS: { id: string; label: string; origins: MediaOrigin[] }[] = [
...
/** A file the browser rejected before it ever reached the server. */
...
export function createMediaPanel(deps: MediaPanelDeps): MediaPanelView {
  const { state, client } = deps;
  const disposer = new Disposer();

  let tabId = "yours";
  /** Refusals, kept until dismissed. Not state the server knows about. */
  let refusals: LocalRefusal[] = [];
  ...
  const node = el(
    "section",
    { class: "panel media", "aria-label": "Media" },
    el(
      "header",
      { class: "panel__header" },
      el("span", { class: "label" }, "Media"),
      el("div", { class: "spacer" }),
      tabs,
      el(
        "button",
  ...
  async function upload(files: FileList | File[]): Promise<void> {
    for (const file of Array.from(files)) {
      ...
      const asset = await attempt(() =>
        client.uploadMediaWithProgress(state.projectId, file, pending.progress),
        {
  ...
  disposer.add(
    ...
    on(node, "dragleave", () => node.classList.remove("is-dropping")),
    on(node, "drop", (event) => {
    ...
    tabs.replaceChildren(
      ...TABS.map((tab) =>
        el(
          ...
          el(
            "span",
            { class: "media__count tabular muted" },
            String(
              tab.origins.reduce((sum, origin) => sum + (counts[origin] ?? 0), 0),
            ),
          ),
        ),
      ),
  ...
  function paintGrid(): void {
    ...
    grid.replaceChildren(
      ...refusalCards,
      ...assets.map((asset) => card(asset)),
      dropTarget(),
    );
  }

  function card(asset: MediaAsset): HTMLElement {
    ...
    const node = el(
      ...
      el(
        ...
        asset.duration_seconds
          ? el(
              "span",
              { class: "media__duration tabular" },
              clock(asset.duration_seconds),
            )
          : null,
        !asset.usable
          ? el(
              "span",
              { class: ["media__state label", asset.status === "refused" && "is-bad"] },
              stateWord(asset),
            )
          : null,
        mark ? el("span", { class: "media__mark label" }, "Project mark") : null,
        deleteButton,
      ),
      el(
        "figcaption",
        { class: "media__caption" },
        el("span", { class: "media__name" }, asset.filename),
        originBadge(asset.origin),
      ),
      asset.detail ? el("p", { class: "media__detail" }, asset.detail) : null,
    );
   ...
   *    about.
   */
  async function deleteAsset(asset: MediaAsset, anchor: HTMLElement): Promise<void> {
    if (deleting.has(asset.media_asset_id)) return;
    ...
    try {
      const removed = await attempt(
        () => client.deleteMedia(state.projectId, asset.media_asset_id),
        { onError: (message) => refuseAt(anchor, message) },
      );
      if (!removed) return;
   ...
   */
  function stateWord(asset: MediaAsset): string {
    if (asset.status === "refused") return "Not usable";
    if (asset.status === "uploading") return "Uploading";
    if (asset.status === "ingesting") return "Reading the file";
    return "Not usable";
  }

  function refusalCard(item: LocalRefusal): HTMLElement {
    return el(
      "figure",
      { class: "media__card is-refused", role: "alert" },
      el(
        "div",
        { class: "media__thumb" },
        el("span", { class: "media__state label is-bad" }, "Not accepted"),
      ),
      el(
        "figcaption",
        { class: "media__caption" },
        el("span", { class: "media__name" }, item.filename),
      ),
      el("p", { class: "media__detail" }, item.sentence),
      el(
        "button",
        {
          class: "btn btn--ghost btn--sm",
  ...
  function pendingCard(file: File): {
    node: HTMLElement;
    ...
    const node = el(
      "figure",
      { class: "media__card is-pending" },
      el(
        "div",
        { class: "media__thumb" },
        label,
        el("span", { class: "media__track" }, bar),
      ),
      el(
        "figcaption",
        { class: "media__caption" },
        el("span", { class: "media__name" }, file.name),
        el("span", { class: "muted tabular" }, bytes(file.size)),
      ),
    );
    return {
      node,
  ...
  function dropTarget(): HTMLElement {
    return el(
      ...
      el("span", null, "Drop files"),
      el("span", { class: "muted" }, "or browse"),
    );
  }

  disposer.add(
    state.media.subscribe(() => {
      paintTabs();
      paintGrid();
    }),
...
export interface AssetDetailOptions {
  ...
  isProjectMark: boolean;
  /** Where the playhead is, for "place at the playhead". */
  playhead: number;
  onUse(lock: boolean): Promise<void>;
  onChange(asset: MediaAsset): void;
  onPlace(kind: TrackKind, at: number): Promise<void>;
 ...
 */
export function assetDetail(options: AssetDetailOptions): HTMLElement {
  const { asset } = options;
  const disposer = new Disposer();
  ...
  if (asset.capabilities.crop) controls.push(cropControls(options, disposer));
  if (asset.capabilities.trim) controls.push(trimControls(options, disposer));
  if (asset.kind === "video") controls.push(sourceAudioControl(options, disposer));
  if (asset.capabilities.level) controls.push(levelControls(options, disposer));
      ...
      asset.url && asset.kind === "video"
        ...
        : asset.url && asset.kind === "audio"
          ...
          : asset.url
            ...
            : el("div", { class: "assetdetail__blank label" }, "No preview"),
    ),
    el(
      "dl",
      { class: "facts facts--tight" },
      row("Kind", `${humanise(asset.kind)} · ${asset.content_type}`),
      asset.width && asset.height
        ? row("Size", `${asset.width} × ${asset.height} · ${bytes(asset.size_bytes)}`)
        : row("Size", bytes(asset.size_bytes)),
      row(
        "Rights",
        asset.origin === "user_upload"
          ? "Yours — no third-party licence needed"
          : `${asset.provenance.licence || "unknown"} · ${asset.provenance.source_name || "unattributed"}`,
      ),
      row(
        "Used in",
        asset.used_by_unit_ids.length === 0
          ? "Not used yet"
          : `${asset.used_by_unit_ids.length} visual(s)`,
      ),
    ),
    ...controls,
    useSection(options, lockToggle),
    placementSection(options),
    el(
      "div",
      { class: "assetdetail__danger" },
      el(
        "button",
        {
          ...
          onclick: () => {
            void (async () => {
              ...
              if (confirmed) await options.onDelete();
            })();
          },
        },
        "Delete",
...
function useSection(
  ...
  if (options.selectedUnitIndex === null) {
    return el(
      "p",
      { class: "assetdetail__lede muted" },
      "Select a visual first, then use this file for it. Or place it on a lane below.",
    );
  }
  ...
  return el(
    "div",
    { class: "assetdetail__usebox" },
    el("span", { class: "label" }, "Use this asset"),
    el(
      "p",
      { class: "assetdetail__lede" },
      `As ${label}. `,
      "Your file becomes the selected version; the one it replaces is kept and ",
      "can be switched back for free.",
    ),
    el(
      "label",
      { class: "checkline", for: "media-lock" },
      lockToggle,
      el("span", null, "Lock so re-planning never replaces it"),
    ),
    el(
      "button",
      {
 ...
 * is not.
 */
function placementSection(options: AssetDetailOptions): HTMLElement {
  const { asset } = options;
  const at = options.playhead;
  const targets: { kind: TrackKind; label: string }[] = [];

  if (asset.capabilities.overlay) targets.push({ kind: "overlay", label: "Overlay" });
  ...
  return el(
    "section",
    { class: "assetdetail__section" },
    el("span", { class: "label" }, "Or place on the timeline"),
    el(
      "p",
      { class: "muted" },
      "Drag it onto a lane, or place it at the playhead:",
    ),
    el(
      "div",
      { class: "assetdetail__places" },
      ...targets.map((target) =>
        el(
          "button",
          {
    ...
    el(
      "p",
      { class: "muted" },
      "Captions and Narration come from your script — nothing can be dropped ",
      "on them.",
    ),
  );
}
...
function row(name: string, value: string): HTMLElement {
  return el(
    ...
    el("dd", { class: "facts__value" }, value),
  );
}
...
function cropControls(
  ...
  const apply = async (): Promise<void> => {
    const updated = await attempt(() =>
      options.client.updateMedia(options.projectId, asset.media_asset_id, {
        crop: {
    ...
    if (updated) options.onChange(updated);
  };
  ...
  for (const field of [x, y, width, height]) {
    disposer.add(
      on(field.input, "input", () => describe()),
      on(field.input, "change", () => void apply()),
    );
  }
  ...
  return el(
    "section",
    { class: "assetdetail__section" },
    el("span", { class: "label" }, "Crop"),
    el(
      "div",
      { class: "assetdetail__fields assetdetail__fields--four" },
      x.node,
      y.node,
      width.node,
      height.node,
...
function trimControls(
  ...
  return el(
    ...
    el(
      "p",
      { class: "muted tabular" },
      `${clock(asset.duration_seconds ?? 0)} selected of ${clock(total)}`,
    ),
  );
}
...
function sourceAudioControl(
  ...
  return el(
    ...
    el(
      "label",
      { class: "checkline", for: "media-source-audio" },
      toggle,
      el("span", null, "Play this clip's own audio"),
    ),
    el(
      "p",
      { class: "muted" },
      "Off by default — your narration owns the sound.",
    ),
  );
}
...
function levelControls(
  ...
  disposer.add(
    ...
    on(level, "change", () => void apply()),
    on(duck, "change", () => {
      describeDuck();
      void apply();
    }),
    on(duckDepth, "input", () => describeDuck()),
    on(duckDepth, "change", () => void apply()),
    on(loop, "change", () => void apply()),
  );

  return el(
    "section",
    { class: "assetdetail__section" },
    el("span", { class: "label" }, "Level"),
    el("div", { class: "assetdetail__slider" }, level, readout),
    el(
      "label",
      { class: "checkline", for: "media-duck" },
      duck,
      el("span", null, "Duck under the narration"),
    ),
    el("div", { class: "assetdetail__slider" }, duckDepth, duckReadout),
    el(
      "label",
      { class: "checkline", for: "media-loop" },
      loop,
      el("span", null, "Loop to fill the video"),
    ),
    el("p", { class: "muted" }, "Your audio is never replaced by a re-plan."),
  );
}
...
function markControls(
  ...
  const promote = async (kind: "logo" | "image"): Promise<void> => {
    const updated = await attempt(() =>
      options.client.updateMedia(options.projectId, asset.media_asset_id, { kind }),
    );
    if (updated) options.onChange(updated);
  };
  ...
  if (!isMark) {
    return el(
      "section",
      { class: "assetdetail__section" },
      el("span", { class: "label" }, "Project mark"),
      el(
        "p",
        { class: "muted" },
        "A mark rides every scene as an overlay. One per project.",
      ),
      el(
        "button",
        {
  ...
  const placement = el("select", { class: "input" }) as HTMLSelectElement;
  for (const [value, label] of [
    ...
    placement.append(
      el("option", { value, selected: asset.logo.placement === value }, label),
    );
  }
  ...
  const opacityOut = el(
    "span",
    { class: "tabular muted" },
    `${Math.round(asset.logo.opacity * 100)}%`,
  );
  ...
  disposer.add(
    on(placement, "change", () => void apply()),
    on(width, "input", () => {
      widthOut.textContent = `${width.value}%`;
    }),
    on(width, "change", () => void apply()),
    on(inset, "change", () => void apply()),
    on(opacity, "input", () => {
  ...
  return el(
    ...
    el(
      "label",
      { class: "field" },
      el("span", { class: "label" }, "Corner"),
      placement,
    ),
    el("span", { class: "label" }, "Width"),
    el("div", { class: "assetdetail__slider" }, width, widthOut),
    el(
      "label",
      { class: "field" },
      el("span", { class: "label" }, "Safe margin (px)"),
      inset,
    ),
    el("span", { class: "label" }, "Opacity"),
    el("div", { class: "assetdetail__slider" }, opacity, opacityOut),
    el(
      "button",
      {
        class: "btn btn--ghost btn--sm",
...
function countsByOrigin(assets: MediaAsset[]): Partial<Record<MediaOrigin, number>> {
  const counts: Partial<Record<MediaOrigin, number>> = {};
 ...
 * "thumbnail" is.
 */
function thumbGlyph(asset: MediaAsset): HTMLElement[] {
...
function waveGlyph(): SVGElement {
  ...
  node.innerHTML = bars
    .map(
      (height, index) =>
        `<rect x="${index * 4 + 1}" y="${12 - height / 2}" width="2" height="${height}" fill="currentColor"/>`,
    )
    .join("");
  return node;
}
...
export function chooseFromLibrary(
  ...
  return new Promise((resolve) => {
    ...
    function renderItem(asset: MediaAsset): HTMLElement {
      return el(
        "button",
        {
          class: "libpick__item",
        ...
        el("div", { class: "media__thumb libpick__thumb" }, ...thumbGlyph(asset)),
        el(
          "div",
          { class: "libpick__meta" },
          el("span", { class: "libpick__name" }, asset.filename),
          el(
            "span",
            { class: "muted tabular" },
            asset.duration_seconds
    ...
    async function handleUpload(file: File): Promise<void> {
      const status = el(
        "p",
        { class: "muted tabular" },
        `Uploading ${bytes(0)} of ${bytes(file.size)}`,
      );
      list.replaceChildren(status);

      const asset = await attempt(
        () =>
          client.uploadMediaWithProgress(projectId, file, (sent, total) => {
    ...
    async function load(): Promise<void> {
      ...
      const library = await attempt(() => client.listMedia(projectId), {
        onError: (message) =>
          list.replaceChildren(inlineError(message, { label: "Try again", onClick: () => void load() })),
      });
      if (!library) return;
