...
export interface PreviewDeps {
  ...
  /** Start a full render — the only way to make the preview current. */
  render(): void;
}

export interface PreviewView {
  node: HTMLElement;
  /** Reload the video source after a render completes. */
  reload(): void;
  play(): void;
  pause(): void;
  toggle(): void;
 ...
 * itself.
 */
function parseAspectRatio(raw: string | null | undefined): {
  ratio: string;
  value: number;
} {
  ...
  if (!w || !h) return { ratio: "16 / 9", value: 16 / 9 };
  return { ratio: `${w} / ${h}`, value: w / h };
}

export function createPreview(deps: PreviewDeps): PreviewView {
  ...
  const stage = el(
    "div",
    {
      ...
      role: "group",
      "aria-label":
        "Video player. Space or K plays or pauses. J or L skips ten seconds. " +
        "The arrow keys nudge five seconds. F toggles fullscreen, M mutes, " +
        "and the number keys jump to that tenth of the video.",
      onclick: () => handleStageClick(),
      ondblclick: (event: MouseEvent) => handleStageDblClick(event),
      onkeydown: (event: KeyboardEvent) => handleStageKeydown(event),
    },
    video,
    glyph,
    placeholder,
  ...
  const playButton = el(
    "button",
    { class: "preview__play", type: "button", "aria-label": "Play" },
    "▶",
  );
  ...
  const scrubTrack = el(
    "div",
    {
      ...
      "aria-valuemax": "100",
      "aria-valuenow": "0",
      onpointerdown: (event: PointerEvent) => beginScrub(event),
      onpointermove: (event: PointerEvent) => continueScrub(event),
      onpointerup: (event: PointerEvent) => endScrub(event),
      onpointercancel: (event: PointerEvent) => endScrub(event),
    },
    scrubBuffered,
    scrubProgress,
    scrubThumb,
  ...
  const controls = el(
    "div",
    { class: "preview__controls" },
    el("div", { class: "preview__scrubrow" }, scrubTrack),
    el(
      "div",
      { class: "preview__buttonrow" },
      stepStart,
      playButton,
      stepEnd,
      clockNow,
  ...
  const node = el(
    "section",
    { class: "panel preview", "aria-label": "Preview" },
    frame,
    controls,
    context,
  );
  ...
  function reload(): void {
    ...
    if (project?.video_url) {
      ...
      placeholder.replaceChildren(
        el("p", { class: "preview__nothing label" }, "Nothing rendered yet"),
        el(
          "button",
          { class: "btn", type: "button", onclick: () => deps.render() },
          "Render preview",
        ),
      );
    }
  ...
  function paintStale(): void {
    ...
    if (moved) {
      staleMark.replaceChildren(
        el("span", { "aria-hidden": "true" }, "◷ "),
        "Timeline changed since this render",
      );
      staleMark.title =
  ...
  disposer.add(
    ...
    on(video, "error", () => {
      ...
      placeholder.replaceChildren(
        el("p", { class: "preview__nothing label" }, "Cannot play here"),
        el("p", { class: "preview__cannot" }, message),
      );
      placeholder.hidden = false;
      video.hidden = true;
    ...
    on(video, "timeupdate", () => {
      ...
      if (!video.paused) state.playhead.set(video.currentTime);
      paintScrub();
    }),
    ...
    on(video, "progress", () => paintScrub()),
    on(video, "volumechange", () => syncVolumeUI()),
  );

  // -- seeking --------------------------------------------------------------

  function scrubDuration(): number {
   ...
   */
  function seekVideoTo(target: number): void {
    const duration = scrubDuration();
    const clamped = clamp(target, 0, duration > 0 ? duration : Infinity);
    if (hasVideo()) video.currentTime = clamped;
    deps.seek(clamped);
  }

  function skipTen(direction: 1 | -1): void {
    if (!hasVideo()) return;
  ...
  function seekToDecile(digit: number): void {
    if (!hasVideo()) return;
  ...
  let glyphHideTimer: number | undefined;
   ...
   * fade.
   */
  function showGlyph(text: string): void {
  ...
  function handleStageDblClick(event: MouseEvent): void {
    ...
    if (x < third) skipTen(-1);
    else if (x > third * 2) skipTen(1);
   ...
   */
  const loadContext = debounce(150, (at: number) => {
        ...
        const parts: Node[] = [
          preview.clip
            ? el(
                "span",
                { class: "tabular" },
                `${preview.clip.label || "Clip"} · ${timecode(preview.clip.start, false)}–${timecode(preview.clip.end, false)}`,
              )
        ...
        if (preview.visual_unit) {
          parts.push(
            el(
              "span",
              { class: "preview__unit" },
              ` · Visual ${String(preview.visual_unit.index + 1).padStart(2, "0")}`,
            ),
          );
        }
        if (lines) {
  ...
  disposer.add(
    ...
    state.project.subscribe(() => reload()),
    state.timeline.subscribe(() => {
