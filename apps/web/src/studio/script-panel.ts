...
import { el, on, revealWithin } from "../core/dom.js";
import { attempt } from "../core/errors.js";
import { clock, plural } from "../core/format.js";
import { Disposer } from "../core/store.js";
import { emptyState, originBadge, skeleton, unitBadge } from "../widgets/states.js";
import type { ScriptBlock, VisualUnit } from "../api/types.js";
...
export interface ScriptPanelDeps {
  state: StudioState;
  client: VtvClient;
  /** Re-plan the visuals, rebuilding the timeline. */
  replan(): Promise<void>;
  /** Try a failed visual again, unchanged. */
  retryUnit(unitId: string): void;
  /** Open the regeneration intents for a failed visual. */
  chooseApproach(unitId: string): void;
  /** Replace a visual with a file already in the library. */
  dropOnUnit(unitId: string, assetId: string): Promise<void>;
  /** Open the revision menu for a scope of blocks. */
  openRevisions(blockIds: string[]): void;
  /** Seek the preview. */
  seek(seconds: number): void;
  /** Paste or type the first script, in an empty project. */
  createScript(text: string): Promise<void>;
}

export interface ScriptPanelView {
  node: HTMLElement;
  refresh(): void;
  dispose(): void;
}

export function createScriptPanel(deps: ScriptPanelDeps): ScriptPanelView {
  ...
  const node = el(
    "section",
    { class: "panel script", "aria-label": "Script" },
    el(
      "header",
      { class: "panel__header" },
      el("span", { class: "label" }, "Script"),
      count,
      el("div", { class: "spacer" }),
      el(
        "button",
  ...
  function paintNotices(): void {
    ...
    if (stale.length > 0) {
      notices.appendChild(
        el(
          "div",
          { class: "notice notice--stale" },
          el("span", { class: "notice__glyph", "aria-hidden": "true" }, "◷"),
          el(
            "span",
            { class: "notice__text" },
            `${plural(stale.length, "visual")} need re-planning after your edits.`,
          ),
          el(
            "button",
            {
      ...
      notices.appendChild(
        el(
          ...
          el(
            "span",
            { class: "notice__text" },
            "Your recording no longer matches this script. The video will use ",
            "your original recording; the captions will follow the script.",
          ),
        ),
      );
    }
  ...
  function paint(): void {
    ...
    const byUnit = new Map<string, VisualUnit>();
    for (const unit of units) {
      for (const blockId of unit.script_block_ids) byUnit.set(blockId, unit);
    }
  ...
  function blockRow(
    ...
    const row = el(
      ...
      el(
        "div",
        { class: "block__gutter" },
        el("span", { class: "block__number tabular" }, String(block.order + 1).padStart(2, "0")),
        el("span", { class: "block__bracket", "aria-hidden": "true" }),
      ),
      el(
        "div",
        { class: "block__main" },
        text,
        startsGroup && unit ? unitStrip(unit) : null,
      ),
    );
    ...
    disposer.add(
      on(text, "click", () => {
        state.selection.set(
          resolveSelection(state, { blockId: block.block_id }),
        );
        const link = state.linkByBlock.get().get(block.block_id);
        if (link) deps.seek(link.start);
        else if (block.start !== null) deps.seek(block.start);
      }),
      on(text, "dblclick", () => beginEdit(block, text)),
  ...
  function unitStrip(unit: VisualUnit): HTMLElement {
    ...
    const node = el(
      "div",
      {
        ...
        onclick: () => {
          state.selection.set(
            resolveSelection(state, { unitId: unit.visual_unit_id }),
          );
        },
      },
      el(
        "span",
        { class: ["block__thumb", `thumb-${unit.status}`] },
        el(
          "span",
          { class: "block__thumb-label label" },
          selected ? shortStrategy(selected.strategy) : "—",
        ),
      ),
      el(
        "span",
        { class: "block__unit-meta" },
        el(
          "span",
          { class: "block__unit-name" },
    ...
    const strip = unit.detail
      ? el(
          "div",
          { class: "block__unitwrap" },
          node,
          el("p", { class: "block__unitdetail" }, unit.detail),
        )
      : node;

    if (unit.status !== "failed") return strip;
  ...
  function beginEdit(block: ScriptBlock, host: HTMLElement): void {
    ...
    const commit = async (): Promise<void> => {
      ...
      const result = await attempt(() =>
        client.updateBlock(state.projectId, block.block_id, next),
      );
      if (!result) {
    ...
    disposer.add(
      ...
      on(area, "input", () => autoGrow(area)),
      on(area, "blur", () => void commit()),
    );
  }
  ...
  function composer(): HTMLElement {
    ...
    return el(
      "div",
      { class: "script__empty" },
      el("p", { class: "label" }, "Your words"),
      el(
        "p",
        { class: "script__empty-lede muted" },
        "Nothing here is rewritten. Improvements arrive as suggestions you ",
        "read and accept, line by line.",
      ),
      area,
      submit,
    );
  ...
  function paintSelection(): void {
    ...
    list.querySelectorAll<HTMLElement>(".block").forEach((row) => {
    ...
    // click.
    if (target && origin !== "script") revealWithin(list, target);
  }

  disposer.add(
    state.script.subscribe(() => {
      paintNotices();
      paint();
    }),
...
function shortStrategy(strategy: string): string {
