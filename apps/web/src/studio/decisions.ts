...
import { clock, delta, humanise, plural } from "../core/format.js";
import { openDialog, type DialogHandle } from "../widgets/dialog.js";
import type {
  PacingPlan,
  RevisionKind,
  RevisionProposal,
...
export function chooseRevision(
  scope: number,
): Promise<{ kind: RevisionKind; language?: string } | null> {
  return new Promise((resolve) => {
    let answer: { kind: RevisionKind; language?: string } | null = null;
    ...
    const handle: DialogHandle = openDialog({
      ...
      body: [
        el(
          "p",
          { class: "dialog__prose" },
          "Nothing changes yet. You will see the exact before and after, with ",
          "what it does to the running time, and decide then.",
        ),
        el(
          "div",
          { class: "revisionkinds" },
        ...
        el(
          "div",
          { class: "revisiontranslate" },
          el("span", { class: "label" }, "Or translate"),
          el(
            "div",
            { class: "revisiontranslate__row" },
            language,
            el(
              "button",
              {
...
export function reviewRevision(options: {
  ...
  onDecided(script: Script, accepted: boolean): void;
  onSuperseded(): void;
}): void {
  ...
  if (proposal.status === "superseded") {
    openDialog({
      ...
      body: el(
        "p",
        { class: "dialog__prose" },
        "Your script changed while this was being prepared. Ask again to get a ",
        "suggestion for the current text.",
      ),
      actions: [
        el(
          "button",
          { class: "btn btn--primary", type: "button", onclick: () => options.onSuperseded() },
          "Try again",
        ),
      ],
    });
    ...
    body: [
      el(
        ...
        el(
          "span",
          { class: "tabular" },
          `${clock(proposal.estimated_duration_before)} → ${clock(proposal.estimated_duration_after)}`,
        ),
        el(
          "span",
          {
          ...
          delta(proposal.duration_delta_seconds),
        ),
        options.affectedUnits > 0
          ? el(
              "span",
              { class: "muted" },
              `Affects ${plural(options.affectedUnits, "visual")} — their timing will need updating.`,
            )
          : null,
      ),
      el(
        ...
        ...changes.map((change, index) =>
          el(
            "div",
            { class: "diff__row" },
            el("span", { class: "diff__number tabular" }, String(index + 1).padStart(2, "0")),
            el(
              "div",
              { class: "diff__pair" },
              el(
                "p",
                { class: "diff__before" },
                el("span", { class: "diff__mark", "aria-hidden": "true" }, "− "),
                change.original,
              ),
              el(
                "p",
      ...
      el(
        "p",
        { class: "diff__note muted" },
        "Accepting replaces every line shown. Your original wording is kept and ",
  ...
  async function decide(accept: boolean): Promise<void> {
    ...
    handle.close();
    if (result) options.onDecided(result.script, accept);
  }
}
  ...
  const handle = openDialog({
    ...
    body: [
      ...
      el(
        ...
        "Shortening opens a suggestion you read before anything changes.",
      ),
    ],
    actions: [
      el(
...
export function disclosuresBeforeExport(options: {
  ...
  const fellBack = options.units.filter((unit) =>
    unit.versions.some(
      (version) =>
        version.version_id === unit.selected_version_id &&
  ...
  if (failed.length) {
    notes.push(
      note(
        `${plural(failed.length, "visual")} failed. Those seconds will hold on the previous shot.`,
      ),
    );
  }
  if (degraded.length) {
    notes.push(
      note(
        `${plural(degraded.length, "visual")} fell back to something other than what was planned.`,
      ),
    );
  }
  if (fellBack.length && !options.hasGenerator) {
    notes.push(
      note(
        `${plural(fellBack.length, "visual")} fell back to typography — no image generator is configured.`,
      ),
    );
  }
  if (options.script?.diverged_from_recording) {
    notes.push(
      note(
        "Captions follow your edited script; the audio says the original words.",
      ),
    );
  }
  ...
  const handle = openDialog({
    ...
    actions: [
      el(
        "button",
        { class: "btn", type: "button", onclick: () => handle.close() },
        "Back",
      ),
      el(
        "button",
  ...
  const handle = openDialog({
    ...
    body: [
      ...
      el(
        "p",
        { class: "dialog__prose muted" },
        "A chart would put numbers on screen that your narration does not give. ",
        "Add the figures to the line, or use a photograph instead.",
      ),
    ],
    actions: [
      el(
...
export function recordingDiverged(options: {
  ...
  const lines =
    options.editedLines.length === 0
      ? "Some lines"
      : options.editedLines.length === 1
        ? `Line ${options.editedLines[0]}`
        : `Lines ${options.editedLines.slice(0, -1).join(", ")} and ${
            options.editedLines[options.editedLines.length - 1]
          }`;
  ...
  const handle = openDialog({
    ...
    body: [
      el(
        "p",
        { class: "dialog__prose" },
        `${lines} changed. The audio still says what you said, and captions `,
        "follow the script — so the two will differ until one of them changes.",
      ),
      el(
        "p",
        { class: "dialog__prose muted" },
        options.canSynthesise
