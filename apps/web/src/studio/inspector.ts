...
import { chooseFromLibrary } from "./media-panel.js";
import type {
  MediaAsset,
  RegenerationIntent,
  VisualUnit,
...
export interface InspectorDeps {
   ...
   *
   * Called only as a fallback, when `useAsVisual` below is not supplied — see
   * that field's comment.
   */
  replaceFromMedia(unitId: string): void;
  /**
   * Apply a library asset as the given unit's visual, exactly as the media
   * panel's own "use as visual" button does. Optional, and not yet supplied
   * by any caller: `createInspector`'s call site in studio.ts needs to pass
   * its existing `useAsVisual(assetId, lock)` function through here — the
   * same one-line wiring `createMediaPanel` already has — for "Replace from
   * Media" to actually swap the visual rather than merely reopening the
   * library panel. Kept optional, rather than required, so this file does
   * not have to edit studio.ts's call site just to compile.
   */
...
export function createInspector(deps: InspectorDeps): InspectorView {
  ...
  function detail(unit: VisualUnit): HTMLElement {
    ...
    return el(
      ...
      el(
        ...
        // Only when there is one. A "Credit: none" row on a drawn visual is
        // noise; a missing credit on a CC-BY photograph is a licence breach,
        // and the asymmetry is the whole reason this row exists.
        selected?.attribution
          ? fact("Credit", [
              el("span", { class: "facts__credit" }, selected.attribution),
            ])
          : null,
  ...
  function actions(unit: VisualUnit): HTMLElement {
    ...
    // Take the previous one out of the body first.
    //
    // `actions()` runs again on every selection, building a fresh menu. Once a
    // menu has been opened it lives in the body rather than in the panel, so it
    // no longer disappears when the panel is rebuilt — without this, selecting
    // four visuals left four detached menus in the document, and the count grew
    // for as long as the Studio stayed open.
    intentMenu?.menu.remove();
    ...
    disposer.add(
      ...
      // The menu is reparented to the body when it opens (see `setMenuOpen`),
      // so it no longer disappears with the panel that built it. Removing it
      // here is what stops one orphaned menu accumulating in the body for
      // every visual the user has ever selected.
      () => menu.remove(),
    );
   ...
   */
  async function pickReplacement(unit: VisualUnit): Promise<void> {
 ...
 */
function setMenuOpen(menu: HTMLElement, button: HTMLElement, open: boolean): void {
  menu.hidden = !open;
  button.setAttribute("aria-expanded", String(open));
  if (open) {
    // Move it to the body before positioning it.
    //
    // `position: fixed` and a z-index are not enough on their own. A fixed
    // element still paints inside the nearest ancestor *stacking context*, and
    // `.studio__right` is `position: sticky` — which creates one
    // unconditionally. So the menu's `z-index: 80` was only ever competing
    // with its siblings inside that column, and the timeline, which is outside
    // it, painted straight over the top: the intent list appeared *underneath*
    // the lanes.
    //
    // No number would have fixed that. A floating layer has to leave the box
    // it was authored in, so it is reparented to the body, where its z-index
    // means what it says. Coordinates are already viewport-relative, so moving
    // it changes stacking and nothing else.
