...
export interface StreamOptions {
  ...
  /** Called with `true` when connected and `false` when the stream drops. */
  ...
  private async run(): Promise<void> {
    while (!this.stopped) {
      try {
        ...
        if (!this.stopped) this.options.onConnection?.(false);
        return;
      } catch (error) {
...
function parseFrame(frame: string): ProjectEvent | null {
  ...
  if (!data) return null;
  try {
    const parsed = JSON.parse(data) as ProjectEvent;
