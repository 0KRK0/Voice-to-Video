...
import { announce, el, fill, on } from "../core/dom.js";
import { attempt, present } from "../core/errors.js";
import { clamp, clock, humanise, plural, quantise } from "../core/format.js";
import { Disposer } from "../core/store.js";
import { readExecution } from "../core/defaults.js";
import { openDialog, refuseAt, toast } from "../widgets/dialog.js";
import { skeleton } from "../widgets/states.js";
import { ProjectEventStream } from "../api/events.js";
import {
  createStudioState,
  loadStudio,
  resolveSelection,
...
import { createScriptPanel } from "../studio/script-panel.js";
import { createPreview } from "../studio/preview.js";
import { createInspector } from "../studio/inspector.js";
import { createTimeline } from "../studio/timeline.js";
import { assetDetail, createMediaPanel } from "../studio/media-panel.js";
import {
  chooseRevision,
  decideOverrun,
  disclosuresBeforeExport,
  groundingRefused,
  recordingDiverged,
  reviewRevision,
} from "../studio/decisions.js";
import { UndoStack, invert } from "../studio/undo.js";
import { SHORTCUTS, installShortcuts } from "../studio/shortcuts.js";
import { buildingPanel } from "./record.js";
import { createTopbar } from "../studio/topbar.js";
import { installPanes } from "../studio/panes.js";
import type {
  PacingMode,
  RegenerationIntent,
  TimelineOperation,
...
const LANE_NAMES: Partial<Record<TrackKind, string>> = {
  music: "Music",
  sfx: "SFX",
  overlay: "Overlay",
  broll: "B-roll",
...
export function studioScreen(
  ...
  async function apply(
    ...
    options: {
      ...
      skipUndo?: boolean;
      /**
       * The element the refusal belongs next to.
       *
       * Passed by the timeline for a drag. Without it a refused move puts the
       * clip back while the sentence explaining why appears in the corner of
       * the screen, and the user reads the picture rather than the corner.
       */
      anchor?: HTMLElement;
    } = {},
  ): Promise<boolean> {
    ...
    try {
      ...
      if (shown.remedy === "reload") {
        ...
        if (options.anchor) refuseAt(options.anchor, shown.message);
        else toast(shown.message, { tone: shown.tone });
      }
      return false;
    }
  }
  ...
  disposer.add(
    state.script.subscribe((script) => {
      if (!script?.diverged_from_recording || warnedAboutDivergence) return;
  ...
  const topbar = createTopbar({
    ...
    onBudget: (usd) => void setSpend({ budget_usd: usd }),
    onFidelity: (fidelity) => void setSpend({ visual_fidelity: fidelity }),
    onStyle: () =>
      ...
      toast(
        "A project's style is fixed when it is created. Set the style for new " +
  ...
  if (jobId) {
    upper.replaceChildren(
      el("div", { class: "studio__building panel" }, building.node),
    );
    ...
    void (async () => {
      try {
        ...
        if (handle.status === "failed") {
          building.fail(
            handle.error?.message ?? "This job did not finish.",
            handle.error?.retryable ?? false,
            handle.error?.detail,
          );
          return;
        }
        ...
        building.fail(
          "Lost contact with the server while this was building. Reload to "
            + "see where it got to — nothing has been thrown away.",
          true,
        );
      }
    })();
  ...
  async function replan(): Promise<void> {
    ...
    if (timelineDoc) state.timeline.set(timelineDoc);
    undo.clear();
    state.saving.set("saved");
  ...
  async function openRevisions(
    ...
    const chosen = forced
      ? { kind: "shorten" as const, language: undefined }
      : await chooseRevision(blockIds.length);
    if (!chosen) return;

    const queued = await attempt(() =>
      client.proposeRevision(projectId, chosen.kind, {
    ...
    const handle = await attempt(() =>
      client.awaitJob(queued.job_id, {
        onTick: (tick) => {
          if (tick.status === "failed") clearBusy(queued.job_id);
        },
      }),
    );
    ...
    if (handle.status !== "ready") {
      toast(
        handle.error?.message ?? "That suggestion could not be prepared.",
        { tone: "warn" },
      );
      return;
    }
    ...
    const proposal = await attempt(() =>
      client.getRevision(projectId, revisionId),
    );
    if (!proposal) return;
   ...
   */
  async function findRevisionId(jobId: string): Promise<string | null> {
    void jobId;
  ...
  async function regenerate(
    ...
    if (unit.locked) {
      toast(
        `Visual ${unit.index + 1} is locked. Unlock it first if you want it regenerated.`,
        { tone: "warn" },
      );
      return;
    }
    ...
    const queued = await attempt(() =>
      client.regenerate(projectId, unitId, intent),
    );
    if (!queued) return;
    ...
    if (handle && handle.status === "failed") {
      toast(
        handle.error?.message ??
          `Visual ${unit.index + 1} could not be regenerated. The previous version still plays.`,
        { tone: "warn" },
      );
    }
  }
  ...
  function patchUnit(
    unitId: string,
    patch: Partial<{ status: string; locked: boolean }>,
  ): void {
    state.units.set(
      state.units.get().map((unit) =>
        unit.visual_unit_id === unitId
          ? ({ ...unit, ...patch } as typeof unit)
          : unit,
      ),
    );
  }

  async function useAsVisual(assetId: string, lock: boolean): Promise<void> {
    ...
    const result = await attempt(() =>
      client.useMediaAsVisual(projectId, unitId, assetId, lock),
    );
    if (!result) return;
    ...
    if (units) state.units.set(units.units);
    if (library) state.media.set(library.assets);
    if (timelineDoc) state.timeline.set(timelineDoc);

    toast(
      lock
        ? "Your file is now this visual, and locked — re-planning will go around it."
        : "Your file is now this visual. It is not locked.",
      { tone: "success" },
    );
  }
   ...
   */
  function useOwnMedia(unitId: string): void {
    const unit = state.unitById.get().get(unitId);
      ...
      fill(
        ...
        ...mine.map((asset) =>
          el(
            ...
            el("span", { class: "ownmedia__name" }, asset.filename),
            el("span", { class: "muted" }, humanise(asset.kind)),
          ),
        ),
      );
    };
    ...
    const send = async (file: File): Promise<void> => {
      ...
      const asset = await attempt(() =>
        client.uploadMediaWithProgress(projectId, file, (sent, total) => {
          status.textContent = `Uploading ${file.name} — ${Math.round(
            (sent / Math.max(1, total)) * 100,
          )}%`;
        }),
      );
      if (!asset) {
    ...
    const handle = openDialog({
      ...
      body: [
        ...
        el(
          ...
          el(
            "button",
            { class: "btn", type: "button", onclick: () => picker.click() },
            "Choose a file",
          ),
        ),
        status,
        ...
        el(
          "label",
          { class: "checkline", for: "phone-lock" },
          lock,
          el("span", null, "Lock it — never replaced by the AI"),
        ),
        camera,
        picker,
      ],
      ...
      body: assetDetail({
        ...
        onChange: (updated) => {
          state.media.set(
            state.media
              .get()
              .map((item) =>
                item.media_asset_id === updated.media_asset_id ? updated : item,
              ),
          );
        ...
        onDelete: async () => {
          const removed = await attempt(() =>
            client.deleteMedia(projectId, assetId),
          );
          if (!removed) return;
          handle.close();
          state.media.set(
  ...
  async function dropMedia(
    ...
    if (kind === "music" || kind === "sfx" || kind === "overlay") {
      if (kind === "overlay" ? !asset.capabilities.overlay : !asset.capabilities.audio_lane) {
        toast(
          `A ${humanise(asset.kind).toLowerCase()} cannot go on the ${LANE_NAMES[kind]} lane.`,
          { tone: "warn" },
        );
        return;
      }
    ...
    if (kind === "caption" || kind === "narration") {
      toast(
        `The ${kind} track comes from your script. Edit the script instead — a ` +
          "change here would be lost the next time the project is re-planned.",
        { tone: "warn" },
      );
      return;
    }
   ...
   */
  async function laneOfKind(kind: TrackKind): Promise<string | null> {
    const existing = state.timeline
      .get()
      ?.tracks.find((item) => item.kind === kind);
    if (existing) return existing.track_id;

    const made = await apply(
      [{ kind: "add_track", track_kind: kind, track_name: LANE_NAMES[kind] ?? kind }],
      { describe: `Add the ${LANE_NAMES[kind] ?? kind} lane` },
    );
    if (!made) return null;
  ...
  /**
   * Change what this project may spend, or what one picture may cost.
   *
   * One function for both because the server takes both in one `PATCH` and
   * because they are one decision. The reload afterwards is not ceremony: the
   * response carries the prices the topbar needs to say what the budget buys,
   * and showing a count computed from a stale price would be a number the
   * render will not honour.
   */
  async function setSpend(spend: {
    ...
    if (!projectId) return;
    try {
      await client.setProjectSpend(projectId, spend);
    } catch (error) {
      const shown = present(error);
    ...
    if ("visual_fidelity" in spend) {
      const tier = spend.visual_fidelity;
      const each = tier ? project?.price_usd?.[tier] : undefined;
      toast(
        tier == null
          ? "Picture quality follows this deployment's default."
          : each
            ? `Picture quality: ${tier}. About $${each.toFixed(3)} a picture.`
            : `Picture quality: ${tier}.`,
        { tone: "info" },
      );
      return;
    }
    const usd = spend.budget_usd;
    toast(
      usd == null
        ? "Budget cleared. This project uses the deployment's ceiling."
        : `Budget set to $${usd.toFixed(2)}. The next render plans against it.`,
      { tone: "info" },
    );
  }

  async function setPacing(
    mode: PacingMode,
    target: number | null,
  ): Promise<void> {
    const result = await attempt(() =>
      client.setPacing(projectId, { pacing: mode, target_seconds: target }),
    );
    if (!result) return;
    state.pacing.set(result.pacing);
    const timelineDoc = await attempt(() => client.getTimeline(projectId));
  ...
  function beforeRender(): void {
    ...
    if (!shown) void startRender("full_project");
  }

  async function startRender(
    scope: "full_project" | "range",
  ): Promise<void> {
    const body =
      scope === "range" &&
      ...
      state.outPoint.get() !== null
        ...
        : { scope: "full_project" as const };

    // Where it runs, from the Computers screen. `auto` is the default and the
    // server resolves it; this only ever asks.
    ...
    const accepted = await attempt(() =>
      client.render(projectId, { ...body, execution }, key),
    );
    if (!accepted) return;

    // Where it *actually* went, in the server's own words. Shown because the
    // whole point of the setting is that a person can tell — an Auto that
    // silently picks the cloud while an expensive graphics card sits idle is a
    // feature nobody trusts twice.
    if (accepted.execution_reason) {
      announce(accepted.execution_reason);
    }
    ...
    markBusy(
      accepted.job_id,
      scope === "range" && accepted.saves_time === false
    ...
    const handle = await attempt(() =>
      client.awaitJob(accepted.job_id, { timeoutMs: 30 * 60_000 }),
    );
    ...
    if (handle?.status === "failed") {
      ...
      toast(
        project.deliverable && project.outcome === "success"
          ? "Rendered."
          : "Rendered with warnings — see the export notes.",
        { tone: project.outcome === "success" ? "success" : "warn" },
      );
    }
  }
  ...
  async function doUndo(): Promise<void> {
    ...
    syncHistory();
    if (ok) toast(`Undid: ${step.describe.toLowerCase()}`, { tone: "info" });
  }
  ...
  function markBusy(jobId: string, label: string): void {
    state.busy.set({ ...state.busy.get(), [jobId]: label });
  }

  function clearBusy(jobId: string): void {
  ...
  const stream = new ProjectEventStream(client.eventsUrl(projectId), {
    ...
    onEvent: (event) => {
      ...
      if (name === "script.revision.proposed") {
        ...
        if (typeof id === "string") lastRevisionId = id;
      }
  ...
  disposer.add(
    installShortcuts({
      ...
      splitAtPlayhead: () => {
        ...
        if (!clipId) return;
        void apply(
          [{ kind: "split", clip_id: clipId, at: state.playhead.get() }],
          { describe: "Split" },
        );
      },
      removeSelected: () => {
          ...
          .then((updated) => {
            state.units.set(
              state.units
                .get()
                .map((item) =>
                  item.visual_unit_id === updated.visual_unit_id ? updated : item,
                ),
            );
      ...
      regenerate: () => {
        const unit = state.selectedUnit.get();
        if (unit) void regenerate(unit.visual_unit_id, "same_idea");
      },
      openRegenerateMenu: () => {
        const button = inspector.node.querySelector<HTMLElement>(
          '.split button[aria-haspopup="true"]',
        );
        button?.click();
      },
      openRevisions: () => {
      ...
      chooseVersion: (index) => {
        ...
        if (!unit || !version || unit.locked) return;
        void client
  ...
  function openHelp(): void {
    ...
    openDialog({
      ...
      body: [
        ...
        el(
          "p",
          { class: "dialog__prose muted" },
          "Everything here is also a visible control. Nothing in this editor is ",
          "keyboard-only, and nothing is drag-only.",
        ),
      ],
    });
  }
...
function boundary(state: StudioState, direction: 1 | -1): number {
  const at = state.playhead.get();
  const edges: number[] = [];
  for (const track of state.timeline.get()?.tracks ?? []) {
    if (track.kind !== "visual") continue;
    for (const clip of track.clips) edges.push(clip.start, clip.end);
  }
  edges.sort((a, b) => a - b);
  if (direction === 1) {
