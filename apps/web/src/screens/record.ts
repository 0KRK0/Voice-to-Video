...
import { el, on, announce } from "../core/dom.js";
import { attempt } from "../core/errors.js";
import { Disposer } from "../core/store.js";
import { openDialog, toast } from "../widgets/dialog.js";
...
const STAGES: { match: string; label: string }[] = [
...
export async function openRecorder(deps: RecorderDeps): Promise<void> {
  ...
  try {
    ...
    toast(
      "We could not open your microphone. Check the browser's permission for this site, or write a script instead.",
      { tone: "error" },
    );
  ...
  const hint = el(
    "p",
    { class: "recorder__hint" },
    "Recording. Take your time — pauses are where the system finds your scenes.",
  );
  ...
  const stopButton = el(
    "button",
    { class: "recorder__stop", type: "button", "aria-label": "Stop and build" },
    el("span", { class: "recorder__stop-mark", "aria-hidden": "true" }),
  );
  ...
  const handle = openDialog({
    ...
    body: el(
      "div",
      { class: "recorder" },
      el("h2", { class: "recorder__title" }, "Explain it out loud"),
      canvas,
      elapsed,
      hint,
      el(
        ...
        stopButton,
        el("span", { class: "label" }, "Stop and build"),
      ),
    ),
    actions: [
      el(
    ...
    on(stopButton, "click", () => {
      void (async () => {
        ...
        const job = await attempt(() =>
          client.uploadRecording(projectId, audio),
        );
        handle.close();
        if (!job) {
          router.navigate("/");
...
export function buildingPanel(): {
  node: HTMLElement;
  apply(eventName: string, data?: Record<string, unknown>): void;
  fail(message: string, retryable: boolean, detail?: string): void;
} {
  const rows = STAGES.map((stage) => {
    ...
    const row = el(
      "li",
      { class: "buildstage", "data-stage": stage.match },
      mark,
      el("span", { class: "buildstage__label" }, stage.label),
      detail,
    );
  ...
  const note = el(
    "p",
    { class: "building__note muted" },
    "You can leave — we will finish without you and keep everything.",
  );

  const node = el(
    "div",
    { class: "building" },
    el("p", { class: "label" }, "Building"),
    el("ol", { class: "buildstages" }, ...rows.map((row) => row.row)),
    note,
  );
  ...
  return {
    ...
    apply(eventName, data) {
      const index = rows.findIndex((row) =>
        eventName.includes(row.stage.match),
      );
      if (index === -1) return;
 ...
 */
function drawWave(canvas: HTMLCanvasElement, stream: MediaStream): () => void {
