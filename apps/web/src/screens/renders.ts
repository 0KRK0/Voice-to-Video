...
import { bytes, clock, humanise, since, timeOfDay } from "../core/format.js";
import { emptyState, skeleton } from "../widgets/states.js";
import { setBaseUrl, signOut } from "../core/session.js";
import {
  ASPECTS,
  PACINGS,
  STYLES,
...
import type { RenderHistoryEntry } from "../api/types.js";
...
export function rendersScreen(
   ...
   */
  async function downloadStoryboard(): Promise<void> {
  ...
  const load = async (): Promise<void> => {
    ...
    fill(
      body,
      running ? currentRender(current) : nothingRunning(),
      history.renders.length
        ? el(
            "section",
            { class: "renders__past" },
            el("h4", { class: "label" }, "Earlier renders"),
            el(
              "table",
              { class: "table" },
              el(
                "tbody",
                null,
                ...history.renders.map((render) =>
                  historyRow(
                    render,
                    projectId,
                    deps,
                    history.downloadable_render_job_id,
                  ),
                ),
              ),
            ),
      ...
      project?.video_url
        ? el(
            "section",
            { class: "renders__sidecars" },
            el("h4", { class: "label" }, "Alongside the video"),
            el(
              "p",
              { class: "muted" },
              "Captions are always written next to the video — never as an ",
              "afterthought and never optional.",
            ),
            el(
              "div",
              { class: "renders__sidecar-actions" },
              el(
                "a",
                {
                  class: "btn btn--sm",
        ...
        : null,
      project?.degradations?.length
        ? el(
            "section",
            { class: "renders__notes" },
            el("h4", { class: "label" }, "What this render shipped with"),
            el(
              "ul",
              { class: "disclosures" },
              ...project.degradations.map((note) =>
                el("li", { class: "disclosures__item" }, note),
              ),
            ),
          )
        : null,
    ...
    if (running) poll = window.setTimeout(() => void load(), 3000);
  };
    ...
    el(
      ...
      el(
        ...
        el(
          "header",
          { class: "panel__header" },
          el("h2", null, "Render & history"),
          el("div", { class: "spacer" }),
          el(
            "a",
            { class: "btn", href: `/studio/${projectId}` },
            "Back to the Studio",
          ),
        ),
        body,
        el(
          "footer",
          { class: "panel__footer muted" },
          "Captions are always written alongside the video, whatever the ",
          "burn-in setting.",
        ),
      ),
    ),
...
function currentRender(render: RenderHistoryEntry): HTMLElement {
  ...
  const reached = Math.min(
    steps.length - 1,
    Math.max(1, Math.round(render.progress * (steps.length - 1))),
  );

  return el(
    "section",
    { class: "renders__current" },
    el(
      "header",
      { class: "renders__head" },
      el("span", { class: "label" }, "This render"),
      el("div", { class: "spacer" }),
      el(
        "span",
        { class: "muted tabular" },
        `Started ${timeOfDay(render.created_at)} · ${render.quality} · ${render.frame_rate}fps`,
      ),
    ),
    el(
      "ol",
      { class: "renders__steps" },
      ...steps.map((step, index) =>
        el(
          ...
          el("span", { class: "label" }, step),
          el(
            "span",
            { class: "renders__step-value tabular" },
            index === reached
    ...
    el(
      "p",
      { class: "renders__note muted" },
      "Progress is what the renderer reports. There is no completion estimate ",
      "we could honestly give.",
    ),
  );
}

function nothingRunning(): HTMLElement {
  return el(
    "p",
    { class: "renders__idle muted" },
    "No render is running.",
  );
}

function historyRow(
  render: RenderHistoryEntry,
  projectId: string,
  deps: RendersDeps,
  ...
  return el(
    ...
    el(
      "td",
      { class: "tabular" },
      `${since(render.created_at)} · ${render.quality}`,
    ),
    el(
      "td",
      { class: ["renders__status", failed && "is-failed"] },
      failed ? "Failed" : render.status === "ready" ? "Ready" : render.status,
    ),
    el("td", { class: "tabular muted" }, clock(render.duration_seconds)),
    el("td", { class: "tabular muted" }, bytes(render.size_bytes)),
    el(
      "td",
      { class: "renders__actions" },
        ...
        : render.has_output
          ? el(
              "span",
              {
                class: "muted",
                title:
                  "Only the most recent render is kept as a file. Render again " +
          ...
          : el(
              "a",
              { class: "btn btn--sm btn--ghost", href: `/studio/${projectId}` },
              "Try again",
            ),
    ),
  );
...
export function settingsScreen(
  root: HTMLElement,
  deps: { client: VtvClient; capabilities: Record<string, boolean> },
): Disposer {
  const disposer = new Disposer();
  ...
  void (async () => {
    ...
    if (!report) {
      fill(
        ...
        el(
          "p",
          { class: "muted" },
          "Usage is not readable with this key. It needs the billing scope.",
        ),
      );
      return;
    }
    ...
    fill(
      ...
      el(
        "p",
        { class: "muted" },
        `Plan ${report.plan} · period ${report.period}`,
      ),
      el(
        "table",
        { class: "table" },
        el(
          ...
          ...rows.map(([kind, entry]) =>
            el(
              "tr",
              null,
              el("td", null, humanise(kind)),
              el("td", { class: "tabular" }, String(entry.used)),
              el(
                "td",
                { class: "tabular muted" },
   ...
   */
  function defaultsForm(): HTMLElement {
    ...
    const make = (
      ...
      for (const value of values) {
        select.append(
          el(
            "option",
            { value, selected: current[key] === value },
            value === "16:9"
              ? "16:9 — landscape"
              : value === "9:16"
                ? "9:16 — vertical"
    ...
    return el(
      ...
      make("Style", "style", STYLES),
      make("Pacing", "pacing", PACINGS),
      make("Aspect", "aspectRatio", ASPECTS),
      el(
        "label",
        { class: "field" },
        el("span", { class: "label" }, "Language"),
        language,
      ),
    );
  }
  ...
  root.append(
    el(
      ...
      el(
        "section",
        { class: "panel page__panel" },
        el("header", { class: "panel__header" }, el("h2", null, "Settings")),
        el(
          "div",
          { class: "panel__body settings" },
          el(
            "section",
            { class: "settings__section" },
            el("h4", { class: "label" }, "This install"),
            el(
              "table",
              { class: "table" },
              el(
            ...
            el(
              "p",
              { class: "muted" },
              "Where a provider is missing, the system draws the visual itself ",
              "and records the fallback on the visual. Nothing silently ",
              "pretends to have used a model it does not have.",
            ),
          ),
          el(
            "section",
            { class: "settings__section" },
            el("h4", { class: "label" }, "Defaults for new projects"),
            defaultsForm(),
            el(
              "p",
              { class: "muted" },
              "These are applied when you create a project. There is no ",
              "account-level store for them on this install, so they are kept ",
              "in this browser — said plainly rather than implying your team ",
              "will inherit them.",
            ),
          ),
          el(
            "section",
            { class: "settings__section" },
            el("h4", { class: "label" }, "Usage"),
            usageBlock,
          ),
          el(
            "section",
            { class: "settings__section" },
            el("h4", { class: "label" }, "Accessibility"),
            el(
              "p",
              null,
              "Motion follows your system setting — with ",
              el("q", null, "reduce motion"),
              " on, transitions and the density animation are switched off ",
              "rather than shortened.",
            ),
            el(
              "p",
              null,
              "Every drag has a keyboard equivalent, and captions are always ",
              "exported alongside the video.",
            ),
            el(
              "p",
              { class: "muted" },
          ...
          el(
            "section",
            { class: "settings__section" },
            el("h4", { class: "label" }, "Connection"),
            el(
              "label",
              { class: "field" },
              el("span", { class: "label" }, "API address"),
              apiField,
            ),
            el(
              "div",
