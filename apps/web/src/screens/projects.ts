...
export function projectsScreen(
  ...
  function matching(): ProjectSummary[] {
    if (!query) return loaded;
    return loaded.filter((item) =>
      (item.title ?? "Untitled").toLowerCase().includes(query),
    );
  }
    ...
    el(
      ...
      el(
        ...
        el(
          ...
          "Deleting a project deletes its recordings, uploads and renders. It ",
          "cannot be undone.",
        ),
      ),
    ),
  ...
  return el(
    ...
    el(
      ...
      el(
        ...
        el("th", null, "Length"),
        el("th", null, "Touched"),
        el("th", { class: "projects__actions-head" }, el("span", { class: "sr-only" }, "Actions")),
      ),
    ),
    el(
      "tbody",
      null,
      ...projects.map((project) => row(project, deps, reload)),
    ),
  );
}
  ...
  return el(
    ...
    el(
      ...
      live
        ...
        : null,
      project.state,
      live && project.progress > 0
        ? el("span", { class: "muted tabular" }, ` · ${Math.round(project.progress * 100)}%`)
        : null,
    ),
    el("td", { class: "tabular" }, clock(project.duration_seconds)),
    el("td", { class: "muted" }, since(project.updated_at)),
    el(
      "td",
      { class: "projects__actions" },
      // Shown always, disabled when this key cannot use it.
      //
      // A key minted with the default `service` role holds no
      // `project:delete`, so this answered "You do not have permission to do
      // that." on every click — the server refusing correctly, the interface
      // having promised something it could not deliver. Hiding it was the
      // first fix and it was worse: the button simply vanished, with nothing
      // to explain where it had gone. Disabled, with the reason in the
      // tooltip, is the honest version — the capability is missing, and the
      // interface says which one and what to do about it.
      el(
        "button",
        {
          class: "btn btn--ghost btn--sm",
          type: "button",
          disabled: !held.has("project:delete"),
          title: held.has("project:delete")
            ? "Delete this project"
            : "This API key cannot delete projects. It was issued with the "
              + "\u2018service\u2019 role, which grants read, create and "
              + "update only. Sign in with a key issued with the "
          ...
          onclick: () => {
            void (async () => {
              const yes = await confirm({
                ...
                body:
                  `“${project.title || "Untitled project"}” and everything in it ` +
              ...
              if (!yes) return;
              await attempt(() =>
                deps.client.deleteProject(project.project_id),
              );
              await reload();
            })();
          },
