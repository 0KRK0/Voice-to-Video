...
export function openDialog(options) {
    ...
    dialog.append(el("form", { method: "dialog", class: "dialog__frame" }, el("header", { class: "dialog__header" }, el("h3", { id: "dialog-title", class: "dialog__title" }, options.title), el("button", {
        class: "dialog__dismiss",
        type: "button",
        "aria-label": "Close",
        onclick: close,
    }, "×")), el("div", { class: "dialog__body" }, ...(Array.isArray(options.body) ? options.body : [options.body])), options.actions?.length
...
export function toast(message, options = {}) {
    ...
    const duration = options.durationMs ?? (tone === "error" ? 0 : tone === "warn" ? 8000 : 4200);
    const node = el("div", { class: ["toast", `toast--${tone}`] }, el("span", { class: "toast__text" }, message), options.action &&
        el("button", {
            class: "btn btn--sm",
            type: "button",
            onclick: () => {
        ...
        }, options.action.label), el("button", {
 ...
 */
export function refuseAt(anchor, sentence) {
    if (!anchor.isConnected) {
        toast(sentence, { tone: "warn" });
        return;
    }
    document.querySelectorAll(".refusal").forEach((node) => node.remove());
    const node = el("div", { class: "refusal", role: "alert" }, el("span", { class: "refusal__text" }, sentence));
    document.body.appendChild(node);
    const place = () => {
        const box = anchor.getBoundingClientRect();
        const own = node.getBoundingClientRect();
        const above = box.top - own.height - 10;
        const left = Math.max(8, Math.min(window.innerWidth - own.width - 8, box.left + box.width / 2 - own.width / 2));
