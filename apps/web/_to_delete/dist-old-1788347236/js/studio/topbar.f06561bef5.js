...
export function createTopbar(deps) {
    ...
    const node = el("header", { class: "topbar" }, el("button", {
        ...
        onclick: () => deps.onBack(),
    }, "‹"), title, saved, el("div", { class: "spacer" }), el("div", { class: "topbar__facts" }, mode.node, style.node, aspect.node, language.node, pacing.node, target.node), busy, el("div", { class: "topbar__actions" }, undoButton, redoButton, el("button", { class: "btn", type: "button", onclick: () => deps.onExport() }, "Export"), renderButton));
