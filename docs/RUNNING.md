...
VTV_IMAGE_GENERATION_MODEL=...    # e.g. gpt-image-1, dall-e-3 — no default
VTV_VIDEO_GENERATION_ENDPOINT=https://.../v1
VTV_VIDEO_GENERATION_API_KEY=...
VTV_VIDEO_GENERATION_MODEL=...    # no default
```

An endpoint and a key alone are not enough for either: with no model set,
`/health` reports the capability as `false` rather than registering a
provider that every request would fail against.
