...
class SuspensionAndAudit(unittest.TestCase):
    def test_a_suspended_tenants_owner_still_cannot_spend(self) -> None:
        ...
        authorizer = Authorizer(record=log, is_suspended={acme}.__contains__)
        with self.assertRaises(Forbidden):
            authorizer.check(
...
class RateLimiting(unittest.TestCase):
    ...
    def test_the_bucket_store_is_bounded(self) -> None:
        """Otherwise the rate limiter is itself the denial of service."""
        from vtv.security.shared_state import InMemorySharedStore

        store = InMemorySharedStore(max_keys=100)
        limiter = RateLimiter(store=store, clock=time.monotonic)
        for index in range(500):
            limiter.check(f"key-{index}")
        self.assertLessEqual(store.size(), 500)
        self.assertGreater(store.size(), 0)

    def test_two_limiters_over_one_store_enforce_one_limit(self) -> None:
        """P0-6. The property the process-local dict could not provide.

        Two `RateLimiter` objects stand in for two API replicas. Before this
        change each held its own buckets, so N pods allowed N times the limit —
        including the credential-stuffing limit.
        """
        from pathlib import Path
        from tempfile import TemporaryDirectory
        ...
        with TemporaryDirectory(prefix="vtv-shared-") as scratch:
            store = SqliteSharedStore(Path(scratch) / "shared.db")
            policy = RateLimitPolicy(rate_per_second=0.001, burst=3)
            now = [1000.0]
            replica_a = RateLimiter(store=store, clock=lambda: now[0])
            replica_b = RateLimiter(store=store, clock=lambda: now[0])

            allowed = 0
            for index in range(10):
                limiter = replica_a if index % 2 == 0 else replica_b
                if limiter.check("org:acme", policy=policy).allowed:
                    allowed += 1

            self.assertEqual(
                allowed, 3, "the two replicas did not share one allowance"
            )

    def test_a_reset_on_one_replica_is_seen_by_the_other(self) -> None:
        from pathlib import Path
        from tempfile import TemporaryDirectory
        ...
        with TemporaryDirectory(prefix="vtv-shared-") as scratch:
            ...
            replica_a = RateLimiter(store=store, clock=lambda: 1000.0)
            replica_b = RateLimiter(store=store, clock=lambda: 1000.0)

            self.assertTrue(replica_a.check("ip:1.2.3.4", policy=policy).allowed)
            self.assertFalse(replica_b.check("ip:1.2.3.4", policy=policy).allowed)
            replica_a.reset("ip:1.2.3.4")
            self.assertTrue(replica_b.check("ip:1.2.3.4", policy=policy).allowed)
