...
class CaptureMeasuresRatherThanTrusts(unittest.TestCase):
    ...
    def test_duration_comes_from_probing_not_from_the_client(self) -> None:
        ...
        recording = run(self.service.capture(organisation_id=SYSTEM_ORGANISATION_ID, project_id="prj_" + "a" * 24, data=data))
    ...
    def test_an_empty_upload_is_refused(self) -> None:
        with self.assertRaises(VTVError):
            run(self.service.capture(organisation_id=SYSTEM_ORGANISATION_ID, project_id="prj_" + "a" * 24, data=b""))
    ...
    def test_an_oversized_upload_is_refused_before_it_is_stored(self) -> None:
        ...
        with self.assertRaises(VTVError) as caught:
            run(service.capture(organisation_id=SYSTEM_ORGANISATION_ID, project_id="prj_" + "a" * 24, data=b"x" * 64))
    ...
    def test_an_over_long_recording_fails_with_a_reason(self) -> None:
        ...
        recording = run(service.capture(organisation_id=SYSTEM_ORGANISATION_ID, project_id="prj_" + "a" * 24, data=data))
    ...
    def test_a_silent_recording_is_caught_before_transcription(self) -> None:
        ...
        recording = run(
            self.service.capture(organisation_id=SYSTEM_ORGANISATION_ID, project_id="prj_" + "a" * 24, data=path.read_bytes())
        )
        self.assertIs(recording.status, Status.FAILED)
        assert recording.error is not None
    ...
    def test_object_keys_are_generated_never_taken_from_the_client(self) -> None:
        ...
        recording = run(self.service.capture(organisation_id=SYSTEM_ORGANISATION_ID, project_id="prj_" + "b" * 24, data=data))
        self.assertIn(recording.recording_id, recording.audio.key)
        # P1-2. The key is generated *and* namespaced: the tenant prefix is
        # what a retention sweep, a deletion and a residency rule all key
        # off, and the storage adapter refuses anything without it.
        self.assertTrue(
            recording.audio.key.startswith(
                f"orgs/{SYSTEM_ORGANISATION_ID}/projects/prj_"
            ),
            recording.audio.key,
        )
