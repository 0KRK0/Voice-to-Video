...
class StubImageGenerationProvider:
    ...
    #:
    #: A count alone cannot catch the failure that matters most here: a
    #: parameter the user chose being dropped somewhere between the studio and
    #: the provider. `calls` goes up either way. This codebase has already lost
    #: a feature to a test double that accepted more than the real thing would
    #: — `async def resolve(self, **_: object)` hid a missing argument for
    #: months — so the stub records what it was actually asked for.
    seen: list[GenerationRequest] = field(default_factory=list)
