...
def measured_duration(path: Path) -> float:
    """Decode the file and time it. Returns 0.0 if even that finds nothing.

    **Why metadata is not enough.** `MediaRecorder` — the browser API behind the
    Speak button — writes WebM for *streaming*: the Duration element lives in a
    header that must be written before the recording has finished, so it is
    simply left out. `ffprobe` then reports no duration on the stream and none
    on the container, both of which are correct readings of a file that does not
    say. Reading only the metadata meant every recording made in a browser
    failed at the capture stage with "audio has no measurable duration" before
    transcription was even reached, while the same audio saved as a `.wav`
    worked perfectly.

    Decoding is the answer rather than a guess: `-f null` runs the whole stream
    through the decoder and discards the output, and the final `time=` it
    reports is the real length. It costs one pass over the audio — milliseconds
    for a few minutes of speech — and it is only reached when the container
    declined to answer.
    """
    completed = run(
    ...
    # `-stats` writes progress to stderr; the last `time=` on it is the end of
    # the stream. Scanned from the end because a long file emits many.
    best = 0.0
    for match in re.finditer(r"time=(\d+):(\d\d):(\d\d(?:\.\d+)?)", completed.stderr or ""):
        hours, minutes, seconds = match.groups()
        best = max(best, int(hours) * 3600 + int(minutes) * 60 + float(seconds))
    return best
...
def probe_audio(path: Path) -> ProbedAudio:
    ...
    if duration <= 0:
        # The container declined to say. Measure it instead — see
        # `measured_duration`, and why every browser recording landed here.
        duration = measured_duration(path)
    if duration <= 0:
        raise VTVError(
            "audio has no measurable duration",
            code=ErrorCode.AUDIO_UNREADABLE,
            user_message=(
                "We could not read that audio. It may be empty, or the "
                "recording may have been cut short before anything was said."
            ),
        )
...
def concat_audio(
    ...
    *,
    ...
    """Join audio files end to end losslessly, and return each one's true length.

    Returns the **measured** duration of every piece after decoding, in order.
    That list is the point of this function: the caller is joining audio whose
    pieces correspond one-to-one with sentences, and it needs to know where each
    sentence starts in the joined file. Summing the returned durations gives
    exactly that, because the join introduces nothing between them.

    ## Why everything is decoded to PCM first

    Two reasons, and the second is the one that matters.

    A container-level `-c copy` concat requires every input to share a codec and
    a layout, which a vendor is under no obligation to guarantee across calls.

    More importantly, **MP3 frames are padded**. An MP3 encoder pads the final
    frame and prepends encoder delay, so joining MP3 files at the container
    level inserts silence at every seam. Measured on this machine, three pieces
    totalling 4.434 s of PCM joined with `-c copy` came to 4.545 s — about
    37 ms per seam. That is inaudible once and ruinous in aggregate: forty
    sentences is roughly a second and a half of drift produced purely by the
    file format, and it accumulates towards the end of the video, which is
    exactly where a viewer notices captions running late. Decoding to PCM and
    joining samples has no seam at all: the same three pieces join to 4.434 s.

    The output is WAV for the same reason: re-encoding the joined result would
    reintroduce the delay at the front of the file.
    """
    if not sources:
        raise VTVError("nothing to join", code=ErrorCode.AUDIO_UNREADABLE)

    target.parent.mkdir(parents=True, exist_ok=True)
    scratch = target.parent / f".{target.stem}-parts"
    scratch.mkdir(parents=True, exist_ok=True)

    decoded: list[Path] = []
    durations: list[float] = []
    for index, source in enumerate(sources):
        part = scratch / f"{index:05d}.wav"
        run(
            [
                FFMPEG, "-y", "-hide_banner", "-loglevel", "error",
                "-i", str(source),
        ...
        decoded.append(part)
        durations.append(round(probe_audio(part).duration_seconds, 3))

    listing = scratch / "parts.txt"
    # Single quotes escaped the way the concat demuxer specifies. These paths
    # are ours, not user input, but a path is still not a format string.
    listing.write_text(
        "\n".join(f"file '{str(p).replace(chr(39), chr(39) + chr(92) + chr(39) + chr(39))}'" for p in decoded)
        + "\n",
        encoding="utf-8",
    )
    run(
