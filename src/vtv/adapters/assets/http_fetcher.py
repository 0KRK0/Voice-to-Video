...
#: Sent on every media download. Wikimedia's user-agent policy is enforced
#: with a 403, not a warning, and their CDN serves a large share of what
#: Openverse indexes — so without this the commons rung finds images it can
#: never actually fetch.
USER_AGENT = (
    "VoiceToVideo/1.0 (openly-licensed media fetch; "
...
class HttpFetcher:
    ...
    async def fetch(self, url: str) -> tuple[bytes, str]:
        ...
        async with httpx.AsyncClient(
            ...
            # every check above.
            #
            # The User-Agent is not politeness. `upload.wikimedia.org` answers
            # **403 Forbidden** to a request without a descriptive one, so every
            # Wikimedia-hosted download failed at this line — including the ones
            # where the search had already found a correctly-licensed
            # photograph. The event said `asset_not_found`, which was true of
