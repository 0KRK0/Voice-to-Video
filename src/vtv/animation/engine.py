...
class AnimationEngine:
    ...
    def __init__(
        ...
        *,
        ...
        #: P1-1. Carried from the Timeline, which carried it from the
        #: Visual Bible. An engine built without them behaves exactly as
        #: before, which is what keeps thumbnails and tests unaffected.
        self.entity_colours = entity_colours or {}

    def theme_for(self, size: RenderSize) -> Theme:
        return Theme.from_style(
            self.style,
            width=size.width,
            height=size.height,
            entity_colours=self.entity_colours,
        )
