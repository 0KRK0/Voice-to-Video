...
#: The reserved tenant that owns internal artefacts: the worked example, the
#: evaluation corpus, `make demo`. A fixed, *real* organisation id rather than
#: a null, so that internal work is subject to exactly the same isolation rules
#: as a customer's — and so that "unowned" never becomes a category the access
#: checks have to interpret.
SYSTEM_ORGANISATION_ID = "prj_0000000000000000000000"
...
class Role(str, Enum):
    ...
    SERVICE = "service"
    #: A customer's own computer, executing leased render jobs. The narrowest
    #: role there is; see `contracts/devices.py` for why it is not an API key.
    DEVICE = "device"
...
class Capability(str, Enum):
    ...
    #: Pair, list and revoke the computers an organisation renders on. An
    #: administrative act: it decides which machines the tenant's material may
    #: be sent to.
    DEVICE_MANAGE = "device:manage"
    #: Draw the frames of a leased job. Deliberately the narrowest capability
    #: in the system, and deliberately *not* enough on its own — a device also
    #: needs a live lease on the specific job. See `contracts/devices.py`.
...
ROLE_CAPABILITIES: dict[Role, frozenset[Capability]] = {
    ...
    Role.ADMIN: frozenset(
        {
            ...
            # Which computers the tenant's material may be sent to is an
            # administrative decision, not an editorial one.
    ...
    # One capability, and not `PROJECT_READ`. A device is sent the timeline for
    # the job it has leased and can reach nothing else — not the project the job
    # belongs to, not the organisation's other projects, not its own history.
    # Everything it is allowed to see arrives in the assignment.
    Role.DEVICE: frozenset({Capability.DEVICE_EXECUTE}),
}
...
class PrincipalKind(str, Enum):
    ...
    SYSTEM = "system"
    #: A paired computer executing a leased job. Authenticated, but able to act
    #: only on the job it holds a lease for.
    DEVICE = "device"
...
class AuditAction(str, Enum):
    ...
    #: A change to a project's content: the script, a visual's state, the
    #: timeline. Distinct from creation and deletion because "who changed my
    #: narration" is a different question from "who deleted my project", and
    #: both get asked.
