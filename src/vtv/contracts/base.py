...
class IdPrefix(str, Enum):
    ...
    # The product layer. Added when the two creation modes — speak and script —
    # were unified onto one model: a script is the narration source of truth in
    # both, and a visual unit is what a user actually directs.
    SCRIPT = "scr"
    SCRIPT_BLOCK = "sbk"
    VISUAL_UNIT = "vun"
    VISUAL_VERSION = "vvr"
    REVISION = "rev"
    TRACK = "trk"
    #: A file the user brought. Distinct from ASSET, which is material the
    #: system acquired and which carries a licence question user media does not.
    MEDIA_ASSET = "mda"

    #: The local-execution layer: a customer's own computer, and the short-lived
    #: code that pairs one to an account.
    DEVICE = "dev"
    PAIRING = "pai"
