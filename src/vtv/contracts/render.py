...
#: Most segment workers to start, however many cores a machine has.
#:
#: Here rather than in the ffmpeg adapter because two adapters need it — the
#: renderer that spawns the pool and the capability report that tells a user
#: what their machine will do — and an adapter importing another adapter is the
#: coupling `test_architecture_boundaries` exists to prevent. A limit is a
#: contract, so it lives with the contracts.
#:
#: Each worker holds one segment's stills in memory and runs one x264, so the
#: ceiling is memory and disk bandwidth rather than arithmetic. Past eight, a
#: 1080p render is waiting on the encoder and the page cache.
...
class RenderJob(RootDocument, Timestamped):
    ...
    #: A playable video of the part that is finished, published while the
    #: render is still running.
    #:
    #: Distinct from `output` on purpose, and never a substitute for it. It
    #: covers only `preview_seconds` of the timeline, so anything that treats
    #: it as the deliverable ships a truncated video — which is why it is a
    #: separate field rather than `output` arriving early.
    preview: ObjectRef | None = None
    #: How much of the timeline `preview` actually contains, in seconds.
    preview_seconds: float = Field(default=0.0, ge=0)
    ...
    #: How many segments each execution target drew, keyed by target value.
    #:
    #: A summary rather than a list, because a four-hour video is about twelve
    #: hundred segments and a per-segment record would be most of the document.
    #: `{"cloud_cpu": 1180, "local_gpu": 20}` answers the question a person
    #: actually has — "did my graphics card do this?" — and the individual
    #: fallbacks are in the event stream with their reasons.
