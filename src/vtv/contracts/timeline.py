...
from vtv.contracts.scale import MAX_SCRIPT_BLOCKS, MAX_TIMELINE_CLIPS
...
class Timeline(RootDocument, Timestamped):
    ...
    #: Where the voice starts on the video clock. Non-zero when the pacing plan
    #: opens on a title card or a held establishing shot.
    narration_start_seconds: float = Field(default=0.0, ge=0.0, le=600.0)
    #: Deliberate visual time *after* the narration ends, allocated by the
    #: pacing planner when the user asked for a video longer than their script.
    #:
    #: This field is why the video is not always exactly as long as the voice.
    #: It was added because it had to be: the planner would produce a perfectly
    #: good `FILLED` plan and the render contract would then refuse every clip
    #: past the end of the narration, so the target-duration feature could plan
    #: but never deliver. Zero by default, so every path that does not ask for
    #: extra time behaves exactly as before.
    fill_seconds: float = Field(default=0.0, ge=0.0, le=3600.0)
    clips: list[VisualClip] = Field(
    ...
    #: P1-1. The Visual Bible's colour lock, flattened onto the document the
    #: renderer actually reads. Before this the Bible was built, reviewed and
    #: then dropped: it had **no effect on any pixel**, which made "visual
    #: consistency" a feature that existed only in a test.
    #:
    #: Keyed by lower-cased entity name; values are `#rrggbb`. It lives on the
    #: Timeline rather than being looked up at draw time because the Timeline is
    #: what gets persisted — so a re-render a month later produces the same
    #: colours even if the Bible has moved on.
    entity_colours: dict[str, str] = Field(default_factory=dict, max_length=200)
    ...
    def _clips_are_ordered_and_bounded(self) -> Timeline:
        limit = self.duration_seconds + TIME_EPSILON
        ...
        for clip in self.clips:
            ...
            if clip.span.end > limit:
                raise ValueError(
                    ...
                    f"{self.duration_seconds:.2f}s video"
                )
        ...
        for cue in self.captions:
            if cue.span.end > limit:
                raise ValueError(
                    f"caption {cue.cue_id} extends past the end of the video"
                )
        return self
    ...
    def duration_seconds(self) -> float:
        ...
        The voice plus whatever deliberate visual time surrounds it. With both
        offsets at their defaults this is exactly the narration duration, which
        is what every path that does not use the target-duration feature gets —
        the voice remains the spine, and nothing here stretches, compresses or
        cuts it.
        """
        return round(
            self.narration_start_seconds
            + self.narration.duration_seconds
            + self.fill_seconds,
            3,
        )

    @property
    def narration_end_seconds(self) -> float:
        """When the voice stops. Everything after this is deliberate fill."""
        return round(
            self.narration_start_seconds + self.narration.duration_seconds, 3
        )
