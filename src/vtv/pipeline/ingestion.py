...
from vtv.contracts.errors import ErrorCode, Status, ValidationFailed
...
class IngestionService:
    ...
    **P1-3.** Two guarantees live here rather than on the HTTP route, because
    the HTTP route is not the only caller and never was: the worker reaches this
    from a queued job, and anything added later — a connector, a batch import, a
    CLI — reaches it too. A defence that only exists in one handler is a defence
    with a hole shaped like the next feature.

    * **Inspection is unconditional.** `inspect_upload` runs on every byte that
      reaches this service. It ran only in `api/app.py` before, so a document
      arriving any other way was parsed unexamined.
    * **Parsing is bounded.** It happens in a child process with a wall-clock
      kill and kernel memory limits (`vtv.ingest_sandbox`), because a hostile
      document's realistic attack is not a malformed header — it is a
      well-formed file that takes four hours.
    """

    registry: object  # ParserRegistry
    events: EventSink
    #: What this service will accept. Documents and text only: an ingestion
    #: service handed a video has been called by mistake, and guessing is worse
    #: than refusing.
    policy: UploadPolicy = field(
        default_factory=lambda: UploadPolicy(
            max_bytes=MAX_DOCUMENT_BYTES,
            allowed_classes=frozenset(
                {ContentClass.DOCUMENT, ContentClass.TEXT, ContentClass.ARCHIVE}
            ),
        )
    )
    limits: SandboxLimits = field(default_factory=SandboxLimits)
    #: Only a test that is measuring the parsers themselves should set this to
    #: False, and `tests/test_ingestion_sandbox.py` asserts the wiring does not.
    sandboxed: bool = True
    ...
    def ingest(
        ...
        *,
        ...
        verdict = inspect_upload(data, policy=self.policy, filename=filename)
        if not verdict.accepted:
            self.events.emit(
                ...
                data={
                    "bytes": len(data),
                    # The verdict's reasons name shapes ("content is text/html,
                    # not the declared application/pdf"), never the content.
                    "reasons": list(verdict.reasons),
                },
            )
            raise ValidationFailed(
                verdict.reasons[0] if verdict.reasons else "that file was refused",
                code=ErrorCode.UPLOAD_REFUSED,
            )

        result: IngestionResult = self._parse(
            data,
    ...
    def _parse(
        ...
        *,
        ...
        if not self.sandboxed:
            return self.registry.parse(  # type: ignore[attr-defined, no-any-return]
                data,
                organisation_id=organisation_id,
                project_id=project_id,
                filename=filename,
